...
public class Helper {
    ...
    public static String recorrido4(JTable tabla1) {
        ...
        aux = aux + Helper.recorridoHaciaArriba(m, 0, nf - 1, 1);
        aux = aux + Helper.recorridoDiagonalPrincipalHaciaAbajo(m, 0, nc / 2 + 1);
        aux = aux + Helper.recorridoDiagonalSecundariaHaciaArriba(m, nc / 2 - 1, 0);
        aux = aux + Helper.recorridoHaciaAbajo(m, nc - 1, 1, nf - 1 + 1);
        return aux;
    }

    public static String recorridoEnCaracol(JTable tabla1) {
        int m[][] = pasoDeDatos(tabla1);
        int nf = m.length;
        int nc = m[0].length;
        String aux = "";
        ...
        for (int i = 1; i < m.length; i++) {
            aux = aux + Helper.recorridoHaciaDerecha(m, nc / 2, nc / 2, nc / 2 + i);
        }
        return aux;
